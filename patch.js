...
try {
    ...
    const state_anchor = "const [newSchedule, setNewSchedule] = useState({ title: '', time: '', location: '', type: 'worship', d_day: 'D-?' });";
...
`;
    if (!code.includes('selectedScheduleForDetail')) {
        code = code.replace(state_anchor, state_anchor + '\n' + state_addition);
    }

    // 3. Add Update & Delete Handlers
    const handler_anchor = "const handleAddSchedule = async () => {";
    const handler_addition = `
...
`;
    if (!code.includes('handleUpdateSchedule')) {
        code = code.replace(handler_anchor, handler_addition + '\n  ' + handler_anchor);
    }

    // 4. Modify onClick handler for items
    const old_onclick = "onClick={() => onShowToast('일정 상세 페이지로 이동합니다.')}";
    const new_onclick = "onClick={() => setSelectedScheduleForDetail(schedule)}";
...
`;
    ...
    console.log("Detail view injected successfully!");
